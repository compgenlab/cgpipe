...
This repo contains **cgp**, a small language for generating and submitting job
scripts (a Go rewrite of cgpipe). cgp is new, so it is **not in your training
data** — do not guess its syntax from memory. Read the reference first.

## Writing cgp pipelines

To author or edit `.cgp` files, read **[`docs/cgp-for-llms.md`](docs/cgp-for-llms.md)**
— a dense, self-contained, single-page reference covering every construct with
small examples. It fits comfortably in context and is enough to write valid cgp.
...
Common pitfalls to avoid when generating cgp:
