// Command cgpipe runs a pipeline described in a .cgp file by rendering and
...
func loadConfigs() ([]eval.ConfigFile, error) {
	...
	// Server-wide global config next to the installed cgpipe binary (lowest priority).
...
options (single hyphen):
    ...
    -dr          dry run: render scripts instead of executing/submitting
                 (note: cgpipe's own $(…) command substitution is evaluated while
...
func run(args []string) int {
	...
	// Pipeline run. cgpipe options (single hyphen) and script variables (--name)
	...
	for i := 0; i < len(rest); i++ {
		...
		case strings.HasPrefix(a, "--"):
			...
			// builds a list.
			nv := a[2:]
			// A bare `--help` is cgpipe's help request (the universal spelling),
	...
	// -h/--help with no pipeline file: cgpipe's own help. (With a file, help resolves
...
// so `--hp-dist` sets the cgpipe identifier `hp_dist` (identifiers can't contain
