...
*single* submission. Instead of N calls to `sbatch`, cgpipe sends one
...
case "$SLURM_ARRAY_TASK_ID" in
  ...
  *) echo "cgpipe: no array task $SLURM_ARRAY_TASK_ID" >&2; exit 1 ;;
esac
```
...
to the element's **task index** and cgpipe coalesces the whole fan-out into one array
submission:
...
cgpipe submits the three `calls.*.vcf` targets as one `--array=1-3` (the same Form-1
...
- **Submission-compatible.** All elements must share their `job.*` settings (mem,
  ...
  for more memory) is an error — cgpipe won't silently submit it under the wrong
...
up to date, cgpipe submits `--array=1,3` and the gather depends only on those two
...
upstream task *i*) needs the scheduler's `aftercorr`, which cgpipe does **not** wire
...
[§15.1](language-spec.md#151-cgpipe-sub--one-off-submission).
