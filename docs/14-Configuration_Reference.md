# Configuration Reference

cgpipe reads layered configuration before your pipeline runs. This is where
site-wide and personal defaults live — a scheduler choice, an account, container
settings — so your pipelines stay portable and cluster specifics stay out of
them. Every config file is itself an ordinary cgpipe script.

## Where config lives

User state lives under one root, `~/.cgpipe/`:

| Path | Purpose |
|------|---------|
| `<cgpipe dir>/.cgpiperc` | Server-wide config next to the installed `cgpipe` binary (lowest priority) |
| `/etc/cgpipe/config` | System (site-wide) config |
| `~/.cgpipe/config` | User config (a cgpipe script) |
| `~/.cgpipe/custom_template.cgp` | Custom submission template for the active scheduler runner |
| `~/.cgpipe/cache/` | Cache / state |

## Resolution order

Each layer overrides the ones before it. A plain `=` in the pipeline always wins;
a `?=` respects whatever an earlier layer set.

1. Built-in defaults
2. Global config next to the binary (`<cgpipe dir>/.cgpiperc`)
3. System config (`/etc/cgpipe/config`)
4. User config (`~/.cgpipe/config`)
5. Environment (`CGPIPE_ENV`, `CGPIPE_RUN_ID`, `CGPIPE_DRYRUN`)
6. Command-line `--name value`
7. The pipeline script

This is why `threads ?= 4` in a pipeline yields to a `--threads 16` on the command
line (layer 6 beats layer 7's `?=`), while `cgpipe.runner = "slurm"` in a pipeline
overrides a config default.

## Environment variables

| Variable | Effect |
|----------|--------|
| `CGPIPE_ENV` | Evaluated as a cgpipe config layer (e.g. `CGPIPE_ENV='cgpipe.runner="slurm"'`) |
| `CGPIPE_RUN_ID` | Sets the run identifier (same as `cgpipe.run_id`) |
| `CGPIPE_DRYRUN` | Any value turns on dry-run mode (same as `-dr`) |

## Selected `cgpipe.*` settings

| Variable | Purpose |
|----------|---------|
| `cgpipe.runner` | `shell`, `slurm`, `sge`, `pbs`, `batchq`, `graphviz`, `html` |
| `cgpipe.runner.<name>.<setting>` | Runner-specific options |
| `cgpipe.runner.<name>.template` | Path to a custom submission template, replacing the built-in for that scheduler |
| `cgpipe.runner.<name>.global_hold` | Submit every job held until the whole pipeline submits cleanly, then release (scheduler runners; off by default) |
| `cgpipe.runner.sge.parallelenv` | SGE parallel-environment name, used for `-pe <pe> <procs>` when `job.procs > 1` |
| `cgpipe.runner.shell.autoexec` | Shell runner: execute the assembled script instead of printing it (default off) |
| `cgpipe.ledger` | Ledger directory path; enables [cross-run tracking](11-The_Ledger.md) |
| `cgpipe.run_id` | Run identifier |
| `cgpipe.shell` | Default shell for rendered bodies |
| `cgpipe.dryrun` | Set by `-dr` / `CGPIPE_DRYRUN` |
| `cgpipe.container.engine` | `docker`, `singularity`/`apptainer`; unset disables container wrapping |
| `cgpipe.container.*` | Bind mounts, env passthrough, engine opts (see [Containers & GPUs](10-Containers_and_GPUs.md)) |
| `cgpipe.gpu` | Default GPU count for all targets |

Some belt-and-suspenders behaviors are **opt-in**, not default — for instance
`global_hold` (submit every job held until the whole pipeline submits cleanly) and
host-environment capture. Enable them in `~/.cgpipe/config` if you want them; the core
stays small.

## Per-job settings: the `job.*` surface

Per-job settings live under the `job.*` namespace, and carry the prefix
everywhere — set globally as a default or inside a target's directive block:

```
# global default in ~/.cgpipe/config
job.mem = "4G"
```
```
# per-target override, same prefix
out.bam: in.bam {{
    job.mem = "16G"
    --
    ...
}}
```

A global `job.*` setting is captured per target at definition time, so it becomes
the default for every target defined after it unless that target overrides it
(`job.procs` itself defaults to 1).

Common settings: `job.name`, `job.procs`, `job.mem`, `job.walltime`,
`job.stdout`, `job.stderr`, `job.container`, `job.gpu`, plus the assembly flags
`job.shexec`, `job.nopre`, `job.nopost`. See [Running Jobs](08-Running_Jobs.md)
for how each maps onto a scheduler.

## Custom runner templates

The scheduler runners render each job from a template. For most site requirements
the named settings (`job.account`, `job.queue`, `job.qos`, …) and the `job.custom`
directive are enough. To replace the submission script wholesale, scaffold from the built-in and
edit it:

```sh
cgpipe show-template -r slurm > ~/.cgpipe/custom_template.cgp
```

cgpipe then uses `~/.cgpipe/custom_template.cgp` for the active scheduler runner, or set
`cgpipe.runner.<name>.template = "<path>"` for explicit, per-scheduler control. The
explicit key wins over the convention file, which wins over the built-in; only the
template is replaced (submit command and status probes are unchanged). See
[Tutorial 10](tutorials/10-custom-templates.md).

## A typical `~/.cgpipe/config`

```
# Default to our cluster, with an account and sane resource floors.
cgpipe.runner = "slurm"
job.account = "lab123"
job.mem ?= "4G"

# Use containers by default.
cgpipe.container.engine = "singularity"

# Keep a ledger so restarts are cheap and cross-run reuse works.
cgpipe.ledger = "/scratch/${USER}/cgpipe.ledger"
```

Pipelines then start clean — often just `include "defaults.cgp"` for project
specifics on top of this personal baseline.

## Next

- **[Running Jobs](08-Running_Jobs.md)** · **[Containers & GPUs](10-Containers_and_GPUs.md)** · **[The Ledger](11-The_Ledger.md)**

Reference → [language-spec.md §11](language-spec.md#11-configuration).
