...
| `~/.cgp/config` | User config (a cgpipe script) |
...
| Variable | Effect |
|----------|--------|
| `CGP_ENV` | Evaluated as a cgpipe config layer (e.g. `CGP_ENV='cgp.runner="slurm"'`) |
...
cgpipe then uses `~/.cgp/custom_template.cgp` for the active scheduler runner, or set
