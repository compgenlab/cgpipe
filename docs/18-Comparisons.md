...
cgpipe covers ground that Snakemake, Nextflow, and WDL also cover — building a
dependency graph of jobs and running them on a cluster. This chapter is a candid
look at where cgpipe is different, to help you decide whether it fits. It is not a
...
## What's distinctive about cgpipe

**The recipe is shell, and only shell.** A cgpipe body is bash you could paste into a
...
**Output-first, like make.** You declare `output: inputs`, and cgpipe figures out the
...
powerful but is a different way of thinking; cgpipe asks you to learn less.

**One binary, no runtime.** cgpipe is a single static executable. There's no Python
...
**A focused ledger, not a global cache.** cgpipe's [ledger](11-The_Ledger.md) records
