...
```sh
cgp pipeline.cgp | bash          # run locally
cat result.txt

cgp -dr pipeline.cgp             # preview the local bash
cgp -dr -r slurm pipeline.cgp    # preview the SLURM sbatch script
cgp -dr -r sge   pipeline.cgp    # preview the SGE qsub script

cgp -r slurm pipeline.cgp        # actually submit (needs sbatch on PATH)
```
...
resources once and cgp does the per-scheduler mapping.
...
`cgp.runner = "slurm"` in `~/.cgp/config` and your pipelines stay portable.
