# Shared helpers for the mock scheduler binaries used by the cgpipe spec suite.
#
# Each mock sources this file and uses capture_call to record what cgpipe sent it
...
#   CGPIPE_TEST_CAPTURE     directory where capture files are written (absolute)
#   CGPIPE_TEST_JOBID_BASE  starting job id (default 1001)
#   CGPIPE_TEST_RESPONSES   optional dir of canned status responses, keyed by jobid
set -u

if [ -z "${CGPIPE_TEST_CAPTURE:-}" ]; then
    echo "lib.sh: CGPIPE_TEST_CAPTURE is unset" >&2
    exit 2
fi
...
capture_call() {
    kind="$1"; shift
    seq_file="${CGPIPE_TEST_CAPTURE}/.seq.${kind}"
    n=$(( $(cat "$seq_file" 2>/dev/null || echo 0) + 1 ))
    echo "$n" > "$seq_file"
    LAST_SEQ="$n"
    stem="${CGPIPE_TEST_CAPTURE}/${kind}-${n}"
    {
...
next_jobid() {
    f="${CGPIPE_TEST_CAPTURE}/.jobid.seq"
    base="${CGPIPE_TEST_JOBID_BASE:-1001}"
...
# return non-zero (job unknown) otherwise. Absent CGPIPE_TEST_RESPONSES ⇒ every job
# is reported active (exit 0) so cross-run reuse paths can be exercised simply.
serve_response() {
    tool="$1"; jobid="$2"
    if [ -z "${CGPIPE_TEST_RESPONSES:-}" ]; then
        return 0
    fi
    path="${CGPIPE_TEST_RESPONSES}/${tool}/${jobid}"
