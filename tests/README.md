...
script run through the `cgp` binary; its output is diffed against a checked-in
...
./tests/run.sh            # same, building cgp on the fly
...
tests/
  ...
  runners/<sched>/    scheduler fixtures — what cgp submits to slurm/sge/pbs/batchq
```
...
Run `cgp [args] <file>`; compare against sibling golden files:
...
| `<name>.setup.sh`   | no  | prep step run in the workdir before cgp (set input mtimes, seed a ledger); `$CGP` is available |
...
`-r <scheduler>`, and compares what cgp sent against `<name>.expected/`:

| file | meaning |
|------|---------|
| `stdout`        | cgp's stdout (the assigned job ids) |
...
so a first seeding `cgp` run (e.g. for the ledger-reuse fixture) doesn't pollute
the captures under test.

## Determinism

Each fixture runs in a fresh temp dir, so cgp output that embeds the absolute
